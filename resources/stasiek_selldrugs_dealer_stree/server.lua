...
RegisterCommand('cliente', function(source, args, rawcommand)
    ...
    for k, v in pairs(Config.drugs) do
        ...
        if count >= 10 then
            drugToSell.count = math.random(1, 10)
        elseif count > 0 then
        ...
        if ESX.Table.SizeOf(Config.drugs) == drugToSell.i and drugToSell.count == 0 then
            --ESX.showNotification(Config.notify.nodrugs, 1)
            TriggerClientEvent('esx:showNotification', source, Config.notify.nodrugs, 6)
        end
    end
end, false)
