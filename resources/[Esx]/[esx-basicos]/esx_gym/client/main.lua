...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para contratar una ~b~bici')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Ya tienes una ~r~bici")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para abriri el ~b~gym~w~ menú')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para hacer ejercicios de ~g~braso')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							ESX.ShowNotification("Descansa ~r~60 seconds ~w~antes de hacer otro ejercicio.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Necesitas una membresía para hacer este ~r~ejercicio")
						end
					elseif training == true then
						ESX.ShowNotification("Necesitas desacansar...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para hacer ~g~flexiones')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para hacer ~yoga')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Puedes hacer ejercicio...")
	end
end
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Estamos abierto ~g~24~w~ horas/dia. Bienvenido!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Batido de proteínas ($6)', value = 'protein_shake'},
				{label = 'Agua ($1)', value = 'water'},
				{label = 'Almuerzo Deportivo ($2)', value = 'sportlunch'},
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Membresía ($800)', value = 'membership'},
            }
        },
        function(data, menu)
