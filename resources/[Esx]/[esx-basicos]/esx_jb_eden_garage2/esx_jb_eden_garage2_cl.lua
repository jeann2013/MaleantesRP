		...
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if(v.fourrieremecano)then
					labelvehicle = vehicleName..': Depósito exterior'
				elseif (v.state)  then
					labelvehicle = vehicleName..': Entregar'
				else
					labelvehicle = vehicleName..': Sacar'
				end	
			...
			ESX.UI.Menu.Open(
				...
				function(data2, menu2)
					...
					elseif data2.current.value == "rename_vehicle" then
						AddTextEntry('FMMC_KEY_TIP8', "Nombre del vehículo deseado")
...
function reparation(prix,vehicle,vehicleProps)
	...
	local elements = {
		{label = "Entregar el vehículo ("..prix.."$)", value = 'yes'},
		{label = "Ve al mecánico", value = 'no'},
	}
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'no') then
				ESX.ShowNotification('Hazle una visita al mecánico')
			end
...
function StockVehicleMenu(KindOfVehicle)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Tu remolque está en el garaje')
					else
			...
			TriggerEvent('esx:showNotification', 'No estás conduciendo el vehículo.')
		end
	else
		TriggerEvent('esx:showNotification', 'No hay vehículo que entregar.')
	end
end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'El remolque ha entrado en el depósito municipal')
					else
						TriggerEvent('esx:showNotification', 'No se puede almacenar este remolque')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'El vehículo entró en el garaje')
					else
						TriggerEvent('esx:showNotification', 'No se puede almacenar este vehículo')
					end
				end,vehicleProps)
			end
		else
			TriggerEvent('esx:showNotification', 'Usted no es el conductor del vehículo.')
		end
	else
		TriggerEvent('esx:showNotification', 'No hay vehículo para entregar')
	end
end
		...
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if v.fourrieremecano then
					...
					labelvehicle = vehicleName..': Salida'
					table.insert(elements, {label =labelvehicle , value = v.vehicle})
				end
			end
		else
			table.insert(elements, {label ="No hay vehículos para sacar" , value = nil})
		end
		...
		function(data, menu)
			...
			elseif data.current.value ~= nil then
				...
				if not iscaronearth then
					...
					ESX.ShowNotification("No se puede sacar este vehículo, ve a buscarlo!")
				end				
			end
		end,
		function(data, menu)
