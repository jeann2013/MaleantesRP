	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
				...
				TriggerClientEvent('esx:showNotification', source,_U('have_withdrawn', count, item.label))
			else
				TriggerClientEvent('esx:showNotification', source,_U('player_cannot_hold'))
			end
		else
			TriggerClientEvent('esx:showNotification', source,_U('not_enough_in_society'))
		end
	end)
end)
...
AddEventHandler('esx_vehicleshop:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if item.count >= 0 then
			...
			TriggerClientEvent('esx:showNotification', source,_U('have_deposited', count, item.label))
		else
			...
			table.insert(tasks, function(cb)
				...
				if xPlayer then
					if xPlayer.getAccount('bank').money >= v.rent_price then
						...
						TriggerClientEvent('esx:showNotification', source,_U('paid_rental', ESX.Math.GroupDigits(v.rent_price), v.plate))
					else
						TriggerClientEvent('esx:showNotification', source,_U('paid_rental_evicted', ESX.Math.GroupDigits(v.rent_price), v.plate))
