...
AddEventHandler('loffe_animations:openMenu', function()
    ...
    table.insert(elements, {label = Strings['Synced2'], value = 'synced2'})
	...
	ESX['UI']['Menu']['Open']('default', GetCurrentResourceName(), 'animations',
	{
        ...
        if data['current']['value'] == 'synced' then
            ...
            elseif data['current']['value'] == 'synced2' then

                local elements2 = {}
                for k, v in pairs(Config['Synced2']) do
                    table.insert(elements2, {['label'] = v['Label'], ['id'] = k})
                end
                
                ESX['UI']['Menu']['Open']('default', GetCurrentResourceName(), 'play_synced2',
                {
                    title = data['current']['label'],
                    align = 'bottom-right',
                    elements = elements2
                    ...
                    current = data2['current']
                    local allowed = false
                    if Config['Synced2'][current['id']]['Car'] then
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            allowed = true
                        else
                            ESX['ShowNotification'](Strings['Not_In_Car'])
                    ...
                    if allowed then
                        if not Config['Debug'] then
                            ...
                            if distance <= 2.0 then
                                TriggerServerEvent('loffe_animations:requestSynced2', closest, current['id'])
                            else
                                ESX['ShowNotification'](Strings['Noone_Close'])
                            end
                            ...
                            CreateThread(function()
                                ...
                                local anim = Config['Synced2'][id]['Accepter']
            
                                if Config['Synced2'][id]['Car'] then
                                    TaskWarpPedIntoVehicle(target, GetVehiclePedIsUsing(PlayerPedId()), 0)
                                end
                                ...
                                anim = Config['Synced2'][id]['Requester']
                            ...
                            CreateThread(function()
                                local anim = Config['Synced2'][id]['Requester']
                                if anim['Attach'] then
                                    local attach = anim['Attach']
                                    AttachEntityToEntity(PlayerPedId(), target, attach['Bone'], attach['xP'], attach['yP'], attach['zP'], attach['xR'], attach['yR'], attach['zR'], 0, 0, 0, 0, 2, 1)
                                end
                                ...
                                anim = Config['Synced2'][id]['Accepter']
