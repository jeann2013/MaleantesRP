  ...
  while true do
    ...
    if IsPedShooting(plyPed) or IsPedInMeleeCombat(plyPed) then
      ...
      if IsPedInMeleeCombat(plyPed) then 
        TriggerEvent('esx_status:add','stress',self.CombatAdder * 5000)
        Citizen.Wait(1000)
      else
        TriggerEvent('esx_status:add','stress',self.ShootingAdder * 5000)
      end
    else
    ...
    TriggerEvent('esx_status:getStatus','stress',function(status)
      ...
      if status.val >= (self.GetSickAt * 10000) then
        ...
        if not self.IsSick then
          self.IsSick = true
          ESX.ShowNotification("Te estas enfermando y teniendo estrés, tomate algo o relajate.")
        else
          ...
          if status.val >= (self.ExtremelySickAt * 10000) then
            ...
            if not self.ExtremelySick then
              ...
              ESX.ShowNotification("Estas enfermo! Tienes que desestresarte o nos pasara algo!.")
          ...
          elseif self.ExtremelySick then 
            ...
            ESX.ShowNotification("Te empiezas a sentir mejor, sigue asi!.")
          end
        end
      elseif status.val <= (self.GetSickAt * 10000) then
        if self.IsSick then
          self.IsSick = false
          ESX.ShowNotification("Te sientes mas relajado.")
...
NewEvent(true,MFS.DoSmoke,'Stress:Smoke')
NewEvent(true,MFS.DoDrink,'Stress:Drink')
NewEvent(true,MFS.RelaxPlayer,'Stress:Relax')
NewEvent(true,MFS.HealPlayer,'Stress:Heal')
