...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			...
			local vehicleBodyDamage = 1000 - vehicleBodyHealth
			local vehicleEngineDamage = 1000 - vehicleEngineHealth
			local vehicleDamageAVG = (vehicleBodyDamage + vehicleEngineDamage) / 2
			vehicleFixPrice =   math.ceil (vehicleDamageAVG * 15)
			TriggerServerEvent("firas:removemoney", vehicleFixPrice)
			...
			notification("~g~The mechanic repaired your car for $" .. vehicleFixPrice .. " !")
			return
		end
...
RegisterNetEvent('firas:nemn')
AddEventHandler('firas:nemn', function ()
	notification("~r~You don't have enough money (The repair costs ~g~$" .. vehicleFixPrice .. "~r~)")
end)
