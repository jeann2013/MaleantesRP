...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 73) and callActive then
            ...
            SendNotification("You have taken a call")
        ...
        elseif IsControlJustPressed(1, 249) and callActive then
            SendNotification("You have rejected a call")
            callActive = false
        end
        if haveTarget then
...
AddEventHandler("call:callIncoming", function(job, pos)
    ...
    work = job
    target.pos = pos
    SendNotification("Press ~g~X~s~ to take the call or ~g~N~s~ to deny")
end)
...
AddEventHandler("target:call:taken", function(taken)
    ...
    elseif taken == 2 then
        SendNotification("Please wait a moment")
    end
end)
