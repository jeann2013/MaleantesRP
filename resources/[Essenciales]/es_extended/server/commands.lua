TriggerEvent('es:addGroupCommand', 'tp', 'admin', function(source, args, user)
	...
	if x and y and z then
		...
		TriggerClientEvent('chatMessage', source, "SISTEMA", {255, 0, 0}, "Invalid coordinates!")
	end
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SISTEMA', 'Ese trabajo no existe' } })
			end

		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1SISTEMA', 'El jugador no está en línea.' } })
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SISTEMA', 'Uso no válido' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SISTEMA', 'Permisos insuficientes' } })
...
TriggerEvent('es:addGroupCommand', 'setmoney', 'admin', function(source, args, user)
	...
	if target and money_type and money_amount and xPlayer ~= nil then
		...
		elseif money_type == 'black' then
			...
			TriggerClientEvent('chatMessage', _source, "SISTEMA", {255, 0, 0}, "^2" .. money_type .. " ^0 ¡No es un tipo de dinero válido!")
			return
		end
	else
		TriggerClientEvent('chatMessage', _source, "SISTEMA", {255, 0, 0}, "Argumentos inválidos")
		return
	end
