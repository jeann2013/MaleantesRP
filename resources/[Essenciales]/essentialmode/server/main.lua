	...
	PerformHttpRequest(VersionAPIRequest, function(err, rText, headers)
		if err == 200 and rText ~= nil then
			...
			if(not _FirstCheckPerformed)then
				print("\n" .. _Prefix .. " Version actual: " .. _VERSION)
				print(_Prefix .. " Ultima versión: " .. decoded.newVersion .. "\n")
			...
			if not decoded.updated then
				...
				print(_Prefix .. " Tienes todo actualizado!\n")
			end
			...
			if decoded.extra then
				if(show_zap ~= "1")then
					...
					if(decoded.extra ~= "^1Advertisement: Servidor Abierto Y estable, Disfruta")then
						print(decoded.extra)
					end
				end
			end
